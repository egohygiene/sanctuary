...
# Reproducible builds
# -------------------
# Pass --build-arg SOURCE_DATE_EPOCH=<unix-ts> to clamp all layer
# timestamps to a deterministic value (see buildkit build-repro docs:
# https://github.com/moby/buildkit/blob/master/docs/build-repro.md).
#
# Pass --build-arg SNAPSHOT_TIMESTAMP=<YYYYMMDDTHHMMSSZ> to pin all
# apt package sources to a specific Ubuntu snapshot
# (https://github.com/reproducible-containers/repro-sources-list.sh).
#
# Base image pinning
# ------------------
# The default BASE_IMAGE uses a floating tag (:ubuntu) which always
# resolves to the latest Ubuntu-based devcontainer image.  For fully
# reproducible builds, override it with a pinned digest at build time:
#
#   docker buildx build \
#     --build-arg BASE_IMAGE=mcr.microsoft.com/devcontainers/base:ubuntu@sha256:<digest> \
#     ...
#
# Run `docker buildx imagetools inspect mcr.microsoft.com/devcontainers/base:ubuntu`
# to obtain the current digest, then commit it to this file for each
# intentional base-image update.
######################################################################
...
# ---------------------------------------------------------------------------
# SOURCE_DATE_EPOCH clamps all layer timestamps to a deterministic value.
...
# SNAPSHOT_TIMESTAMP pins apt sources to a specific Ubuntu snapshot.
# Format: YYYYMMDDTHHMMSSZ (e.g. 20240101T000000Z)
# See https://github.com/reproducible-containers/repro-sources-list.sh
ARG SNAPSHOT_TIMESTAMP
ENV SNAPSHOT_TIMESTAMP=${SNAPSHOT_TIMESTAMP}
...
# Homebrew (Linuxbrew) installation prefix
...
ENV PATH="${HOMEBREW_PREFIX}/bin:${HOMEBREW_PREFIX}/sbin:${JAVA_HOME}/bin:${UNIVERSAL_BIN}:${UNIVERSAL_BIN}/install:${UNIVERSAL_BIN}/tests:${UNIVERSAL_TOOLBOX}:${TASKFILE_HOME_DIR}:${ASDF_DIR}/bin:${ASDF_SHIMS_DIR}:${PATH}"
...
# ---------------------------------------------------------------------------
# Hardening: deploy security configuration baselines.
#
# All source files live under shell/etc/ and are copied here so that
# login.defs, sysctl tunables, PAM resource limits, sudoers policy,
# AIDE file-integrity config, Lynis audit profile, and systemd unit
# files for periodic checks are all baked into the image from the
# very first layer.
#
# This preserves the "universal" character of the devcontainer:
#   - No interactive tools are removed.
#   - The devcontainer sudoers drop-in keeps NOPASSWD:ALL for %sudo.
#   - AIDE and Lynis configs are installed but not enforced at boot;
#     they are invoked on demand via the hardening/* scripts.
# ---------------------------------------------------------------------------

# sysctl tunables (inotify watches, UDP buffer sizes)
COPY shell/etc/sysctl.d/ /etc/sysctl.d/

# PAM resource limits tailored for Docker-in-Docker workloads
COPY shell/etc/security/limits.d/ /etc/security/limits.d/

# PAM password-quality policy (libpam-pwquality)
COPY shell/etc/security/pwquality.conf /etc/security/pwquality.conf

# Login / account defaults (umask 027, SHA-512 passwords, secure HOME_MODE)
COPY shell/etc/login.defs /etc/login.defs

# Devcontainer-specific sudoers drop-in (NOPASSWD for %sudo group)
COPY shell/etc/sudoers.d/devcontainer /etc/sudoers.d/devcontainer

# AIDE – file integrity monitoring baseline config
COPY shell/etc/aide/aide.conf     /etc/aide/aide.conf
COPY shell/etc/aide/aide.conf.d/  /etc/aide/aide.conf.d/

# Lynis – security audit profile
COPY shell/etc/lynis/default.prf /etc/lynis/default.prf

# systemd units: AIDE daily check, Lynis weekly audit, /tmp hardening mount
COPY shell/etc/systemd/system/ /etc/systemd/system/

# Shell login banner (security notice + environment context shown on login)
COPY shell/etc/profile.d/90-banner.sh /etc/profile.d/90-banner.sh
COPY shell/etc/profile.d/banner.txt   /etc/profile.d/banner.txt

# Lock down permissions on security-sensitive files
RUN chmod 0440 /etc/sudoers.d/devcontainer \
 && chmod 0644 /etc/login.defs \
 && chmod 0644 /etc/security/pwquality.conf \
 && find /etc/security/limits.d -name '*.conf' -exec chmod 0644 {} \; \
 && find /etc/sysctl.d -name '*.conf' -exec chmod 0644 {} \; \
 && chmod 0600 /etc/aide/aide.conf \
 && find /etc/aide/aide.conf.d -type f -exec chmod 0600 {} \; \
 && chmod 0600 /etc/lynis/default.prf \
 && chmod 0644 /etc/systemd/system/aidecheck.service \
 && chmod 0644 /etc/systemd/system/aidecheck.timer \
 && chmod 0644 /etc/systemd/system/lynis.service \
 && chmod 0644 /etc/systemd/system/lynis.timer \
 && chmod 0644 /etc/systemd/system/tmp.mount \
 && chmod 0644 /etc/profile.d/90-banner.sh
...
# ---------------------------------------------------------------------------
# Pin apt sources to a reproducible snapshot (repro-sources-list.sh)
# https://github.com/reproducible-containers/repro-sources-list.sh
#
# If SNAPSHOT_TIMESTAMP is set, all subsequent apt-get calls in this stage
# will use the pinned Ubuntu snapshot, ensuring the same package versions
# are installed regardless of when the image is built.
# ---------------------------------------------------------------------------
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked,id=apt-cache \
    --mount=type=cache,target=/var/lib/apt,sharing=locked,id=apt-lib \
    bash -Eeuo pipefail -c '\
      if [[ -n "${SNAPSHOT_TIMESTAMP:-}" ]]; then \
        echo "📌 Pinning apt sources to snapshot: ${SNAPSHOT_TIMESTAMP}"; \
        "${UNIVERSAL_BIN}/install/repro-sources-list" "${SNAPSHOT_TIMESTAMP}"; \
        apt-get update -qq; \
      else \
        echo "⏭️  SNAPSHOT_TIMESTAMP not set; skipping apt source pinning"; \
      fi'
...
RUN set -Eeuo pipefail; \
    if [ -n "${PYTHON_COMMIT}" ]; then \
      echo "🐍 Cloning CPython repository from ${PYTHON_REPO}…"; \
      git init .; \
      git remote add origin "${PYTHON_REPO}"; \
      echo "📌 Fetching commit ${PYTHON_COMMIT}…"; \
      ...
      if [ -n "${PYTHON_SHA256}" ]; then \
        ...
        echo "⚠️  PYTHON_SHA256 not provided; proceeding without checksum verification"; \
      fi; \
    ...
    echo "✅ CPython ${PYTHON_VERSION} installed successfully at ${PY_PREFIX}."
...
ARG INSTALL_DUST=true
ARG DUST_REPO=https://github.com/bootandy/dust
ARG DUST_COMMIT=fabe19b8de4d1d36ad55dfa28b00698bc55297e6

ENV INSTALL_DUST=${INSTALL_DUST} \
    DUST_REPO=${DUST_REPO} \
    DUST_COMMIT=${DUST_COMMIT}

RUN bash -Eeuo pipefail -c '\
      if [[ "${INSTALL_DUST}" == "true" ]]; then \
        cargo install --git ${DUST_REPO} du-dust --rev ${DUST_COMMIT} --force && \
        dust --version && \
        echo "✅ dust installed successfully." \
      else \
        echo "📁 Skipping dust install"; \
      fi'
# Note: the crate is published as "du-dust" but installs the "dust" binary
...
# E57 (libE57Format) – optional but recommended before PDAL
...
# ------------------------------
FROM pdal AS ceres-solver

# Build-time toggles & pins
ARG INSTALL_CERES_SOLVER=false

# Your version + SHA-512 pin
ARG CERES_VERSION=2.2.0
ARG CERES_SHA512=d4cefe5851e25bd3c7b76352092d8d549eb371af2e35a325736554c54fe58a3523658697c3e2d65af660fe6895ae3d96fe31bd1875870474fc4b6fed3bbdfae9

# Optional override (leave empty to use version pin)
ARG CERES_URL=

ENV INSTALL_CERES_SOLVER="${INSTALL_CERES_SOLVER}" \
    CERES_VERSION="${CERES_VERSION}" \
    CERES_SHA512="${CERES_SHA512}" \
    CERES_URL="${CERES_URL}" \
    CERES_PREFIX="${UNIVERSAL_TOOLBOX}/ceres-solver"

RUN --mount=type=cache,target=/var/cache/apt,sharing=locked,id=apt-cache \
    --mount=type=cache,target=/var/lib/apt,sharing=locked,id=apt-lib \
    --mount=type=cache,target=/var/cache/src,sharing=locked,id=src-cache \
    bash -Eeuo pipefail -c '\
      if [[ "${INSTALL_CERES_SOLVER}" == "true" ]]; then \
        set -x; \
        install-ceres-solver \
          --version "${CERES_VERSION}" \
          ${CERES_URL:+--url "${CERES_URL}"} \
          --sha512 "${CERES_SHA512}" \
          --prefix "${CERES_PREFIX}" \
          --with-deps \
          --symlink-bin \
          --debug && \
        test -f /etc/ld.so.conf.d/ceres-solver.conf && head -n 1 /etc/ld.so.conf.d/ceres-solver.conf; \
      else \
        echo "📁 Skipping Ceres Solver install"; \
      fi'
...
# ------------------------------
FROM cpython AS shdoc

# Build-time toggles & pins
ARG INSTALL_SHDOC=true
...
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked,id=apt-cache \
    ...
    bash -Eeuo pipefail -c '\
      if [[ "${INSTALL_SHDOC}" == "true" ]]; then \
        ...
        echo "✅ shdoc installed to ${SHDOC_PREFIX}"; \
      else \
        echo "📁 Skipping shdoc install"; \
      fi'


# ------------------------------
# git-sim (git command visualizer)
# https://github.com/initialcommit-com/git-sim
# ------------------------------
FROM shdoc AS git-sim

# Build-time toggles & pins
ARG INSTALL_GIT_SIM=true
ARG GIT_SIM_VERSION=""

ENV INSTALL_GIT_SIM="${INSTALL_GIT_SIM}" \
    GIT_SIM_VERSION="${GIT_SIM_VERSION}"

RUN bash -Eeuo pipefail -c '\
      if [[ "${INSTALL_GIT_SIM}" == "true" ]]; then \
        install-git-sim \
          ${GIT_SIM_VERSION:+--version "${GIT_SIM_VERSION}"} && \
        echo "✅ git-sim installed successfully."; \
      else \
        echo "📁 Skipping git-sim install"; \
      fi'

# ------------------------------
# gitinspector (Git repository statistics analyzer)
# https://github.com/ejwa/gitinspector
# ------------------------------
FROM git-sim AS gitinspector

# Build-time toggles & pins
ARG INSTALL_GITINSPECTOR=true
ARG GITINSPECTOR_VERSION=""

ENV INSTALL_GITINSPECTOR="${INSTALL_GITINSPECTOR}" \
    GITINSPECTOR_VERSION="${GITINSPECTOR_VERSION}"

RUN bash -Eeuo pipefail -c '\
      if [[ "${INSTALL_GITINSPECTOR}" == "true" ]]; then \
        install-gitinspector \
          ${GITINSPECTOR_VERSION:+--version "${GITINSPECTOR_VERSION}"} && \
        echo "✅ gitinspector installed successfully."; \
      else \
        echo "📁 Skipping gitinspector install"; \
      fi'
...
# ------------------------------
FROM gitinspector AS sqlfluff

# Build-time toggles & pins
ARG INSTALL_SQLFLUFF=true
ARG SQLFLUFF_VERSION=""

ENV INSTALL_SQLFLUFF="${INSTALL_SQLFLUFF}" \
    SQLFLUFF_VERSION="${SQLFLUFF_VERSION}"

RUN bash -Eeuo pipefail -c '\
      if [[ "${INSTALL_SQLFLUFF}" == "true" ]]; then \
        install-sqlfluff \
          ${SQLFLUFF_VERSION:+--version "${SQLFLUFF_VERSION}"} && \
        echo "✅ sqlfluff installed successfully."; \
      else \
        echo "📁 Skipping sqlfluff install"; \
      fi'
...
# ------------------------------
FROM sqlfluff AS calibre

# Build-time toggles & pins
ARG INSTALL_CALIBRE=false
ARG CALIBRE_VERSION=""
ARG CALIBRE_INSTALL_DIR="/opt/calibre"

ENV INSTALL_CALIBRE="${INSTALL_CALIBRE}" \
    CALIBRE_VERSION="${CALIBRE_VERSION}" \
    CALIBRE_INSTALL_DIR="${CALIBRE_INSTALL_DIR}"

ENV PATH="${CALIBRE_INSTALL_DIR}:${PATH}"
...
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked,id=apt-cache \
    ...
    bash -Eeuo pipefail -c '\
      if [[ "${INSTALL_CALIBRE}" == "true" ]]; then \
        install-calibre \
          --install-dir "${CALIBRE_INSTALL_DIR}" \
          ${CALIBRE_VERSION:+--version "${CALIBRE_VERSION}"} && \
        echo "✅ Calibre installed successfully."; \
      else \
        echo "📁 Skipping calibre install"; \
      fi'

# FROM gcloud AS final
FROM calibre AS dockerfmt
...
# Build-time toggles & pins
ARG INSTALL_DOCKERFMT=true
ARG DOCKERFMT_VERSION=""

ENV INSTALL_DOCKERFMT="${INSTALL_DOCKERFMT}" \
    DOCKERFMT_VERSION="${DOCKERFMT_VERSION}"

RUN bash -Eeuo pipefail -c '\
      if [[ "${INSTALL_DOCKERFMT}" == "true" ]]; then \
        install-dockerfmt \
          ${DOCKERFMT_VERSION:+--version "${DOCKERFMT_VERSION}"} && \
        echo "✅ dockerfmt installed successfully."; \
      else \
        echo "📁 Skipping dockerfmt install"; \
      fi'
...
# ------------------------------
FROM dockerfmt AS mitmproxy

# Build-time toggles & pins
ARG INSTALL_MITMPROXY=true
ARG MITMPROXY_VERSION=""

ENV INSTALL_MITMPROXY="${INSTALL_MITMPROXY}" \
    MITMPROXY_VERSION="${MITMPROXY_VERSION}"

RUN bash -Eeuo pipefail -c '\
      if [[ "${INSTALL_MITMPROXY}" == "true" ]]; then \
        install-mitmproxy \
          ${MITMPROXY_VERSION:+--version "${MITMPROXY_VERSION}"} && \
        echo "✅ mitmproxy installed successfully."; \
      else \
        echo "📁 Skipping mitmproxy install"; \
      fi'
...
# ------------------------------
FROM mitmproxy AS shfmt

# Build-time toggles & pins
ARG INSTALL_SHFMT=true
ARG SHFMT_VERSION=""

ENV INSTALL_SHFMT="${INSTALL_SHFMT}" \
    SHFMT_VERSION="${SHFMT_VERSION}"

RUN bash -Eeuo pipefail -c '\
      if [[ "${INSTALL_SHFMT}" == "true" ]]; then \
        install-shfmt \
          ${SHFMT_VERSION:+--version "${SHFMT_VERSION}"} && \
        echo "✅ shfmt installed successfully."; \
      else \
        echo "📁 Skipping shfmt install"; \
      fi'

# ------------------------------
# talisman (git secret scanner)
# https://thoughtworks.github.io/talisman/docs/installation/global-hook/
# ------------------------------
FROM shfmt AS talisman

# Build-time toggles & pins
ARG INSTALL_TALISMAN=true
ARG TALISMAN_VERSION=""
ARG TALISMAN_HOME="${UNIVERSAL_TOOLBOX}/talisman"

ENV INSTALL_TALISMAN="${INSTALL_TALISMAN}" \
    TALISMAN_VERSION="${TALISMAN_VERSION}" \
    TALISMAN_HOME="${TALISMAN_HOME}"

# Ensure the talisman binary is on PATH
ENV PATH="${TALISMAN_HOME}/bin:${PATH}"

RUN bash -Eeuo pipefail -c '\
      if [[ "${INSTALL_TALISMAN}" == "true" ]]; then \
        install-talisman \
          --talisman-home "${TALISMAN_HOME}" \
          ${TALISMAN_VERSION:+--version "${TALISMAN_VERSION}"} \
          --skip-hook && \
        echo "✅ talisman installed successfully."; \
      else \
        echo "📁 Skipping talisman install"; \
      fi'
...
ARG LINUXBREW_USER=linuxbrew

ENV INSTALL_LINUXBREW="${INSTALL_LINUXBREW}" \
    BREW_USER="${LINUXBREW_USER}"
    ...
    bash -Eeuo pipefail -c '\
      if [[ "${INSTALL_LINUXBREW}" == "true" ]]; then \
        install-linuxbrew \
          ...
          --brew-user "${BREW_USER}" && \
        echo "✅ Homebrew (Linuxbrew) installed successfully."; \
      else \
        echo "📁 Skipping Linuxbrew install"; \
      fi'

FROM linuxbrew AS terminus
...
# Build-time toggles & pins
ARG INSTALL_TERMINUS=true
ARG TERMINUS_VERSION=""
ARG TERMINUS_INSTALL_DIR="/usr/local/bin"
ARG PHP_VERSION="8.3"

ENV INSTALL_TERMINUS="${INSTALL_TERMINUS}" \
    TERMINUS_VERSION="${TERMINUS_VERSION}" \
    TERMINUS_INSTALL_DIR="${TERMINUS_INSTALL_DIR}" \
    PHP_VERSION="${PHP_VERSION}"

# Ensure the terminus binary is on PATH
ENV PATH="${TERMINUS_INSTALL_DIR}:${PATH}"

RUN --mount=type=cache,target=/var/cache/apt,sharing=locked,id=apt-cache \
    --mount=type=cache,target=/var/lib/apt,sharing=locked,id=apt-lib \
    bash -Eeuo pipefail -c '\
      if [[ "${INSTALL_TERMINUS}" == "true" ]]; then \
        install-terminus \
          --install-dir "${TERMINUS_INSTALL_DIR}" \
          --php-version "${PHP_VERSION}" \
          ${TERMINUS_VERSION:+--version "${TERMINUS_VERSION}"} && \
        echo "✅ Terminus installed successfully."; \
      else \
        echo "📁 Skipping Terminus install"; \
      fi'

FROM terminus AS image-optim

# ------------------------------
# image_optim (Ruby gem + binary helpers)
# https://github.com/toy/image_optim
# ------------------------------

# Build-time toggles & pins
ARG INSTALL_IMAGE_OPTIM=true
ARG IMAGE_OPTIM_VERSION=""
ARG IMAGE_OPTIM_BINDIR="/usr/local/bin"

ENV INSTALL_IMAGE_OPTIM="${INSTALL_IMAGE_OPTIM}" \
    IMAGE_OPTIM_VERSION="${IMAGE_OPTIM_VERSION}" \
    IMAGE_OPTIM_BINDIR="${IMAGE_OPTIM_BINDIR}"

RUN --mount=type=cache,target=/var/cache/apt,sharing=locked,id=apt-cache \
    --mount=type=cache,target=/var/lib/apt,sharing=locked,id=apt-lib \
    bash -Eeuo pipefail -c '\
      if [[ "${INSTALL_IMAGE_OPTIM}" == "true" ]]; then \
        install-image-optim \
          --bindir "${IMAGE_OPTIM_BINDIR}" \
          ${IMAGE_OPTIM_VERSION:+--gem-version "${IMAGE_OPTIM_VERSION}"} \
          --no-color && \
        echo "✅ image_optim installed successfully."; \
      else \
        echo "📁 Skipping image_optim install"; \
      fi'

FROM image-optim AS lynis
...
# Build-time toggles & pins
ARG INSTALL_LYNIS=true
ARG LYNIS_VERSION=""

ENV INSTALL_LYNIS="${INSTALL_LYNIS}" \
    LYNIS_VERSION="${LYNIS_VERSION}"

RUN bash -Eeuo pipefail -c '\
      if [[ "${INSTALL_LYNIS}" == "true" ]]; then \
        install-lynis \
          ${LYNIS_VERSION:+--version "${LYNIS_VERSION}"} && \
        echo "✅ lynis installed successfully."; \
      else \
        echo "📁 Skipping lynis install"; \
      fi'

FROM lynis AS final

# ---------------------------------------------------------------------------
# Hardening: remove legacy/unnecessary system accounts that have no role in
# a devcontainer.  This mirrors the iron-debian practice of keeping only the
# accounts that the running process actually needs.
# ---------------------------------------------------------------------------
RUN bash -Eeuo pipefail -c '\
      remove-legacy-users --force --allow-outside && \
      echo "✅ Legacy system users removed."'
