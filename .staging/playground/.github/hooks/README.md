...
1. Add an entry to `ego-hygiene.json` with `"type": "command"`, `timeoutSec`,
   `bash`, `powershell`, and `"cwd": "."`.
1. Create `scripts/copilot-hooks/<hook-name>.sh` (Bash) and
   `scripts/copilot-hooks/<hook-name>.ps1` (PowerShell).
1. Follow the Bash standards: `#!/usr/bin/env bash`, `set -euo pipefail`,
   `printf` over `echo`, shdoc comments, quoted expansions, no `eval`.
1. Add test fixtures to `scripts/copilot-hooks/tests/`.
1. Run `task copilot:hooks:validate` and `task copilot:hooks:test`.
