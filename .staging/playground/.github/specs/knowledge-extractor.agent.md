...
1. `mindgarden/_system/templates/source.md`
1. `mindgarden/_system/templates/knowledge.md`
1. The nearest applicable `AGENTS.md`, repository instructions, and Mind Garden
   documentation

When the task explicitly includes synapse creation, also locate:

1. `.github/specs/synapse-create.spec.md`
1. `mindgarden/_system/templates/synapse.md`
...
1. Applicable repository and agent instructions
1. `knowledge-extract.spec.md`
1. Current Mind Garden templates
1. Existing local conventions that do not conflict with the above
...
1. Use aggregate organization and the portable output profile.
1. Inspect every accessible source in scope.
1. Register inaccessible or partially accessible material.
1. Extract durable, high-signal knowledge with sufficient standalone context.
1. Distinguish sourced information, user statements and decisions, third-party
   claims, AI suggestions, and new inference.
1. Return one knowledge bundle using the specification's standard output
   contract.
1. Do not write, route, archive, delete, publish, or overwrite anything unless
...
1. Preserve the complete knowledge result independently.
1. Pass the extracted knowledge—not an improvised summary—to the synapse stage.
1. Apply `synapse-create.spec.md` and the synapse template.
1. Do not force every knowledge unit into a synapse.
1. Preserve links from synapses back to supporting knowledge and sources.
1. Keep publishing ideas separate from finished publishing artifacts.
...
1. `Decisions, Actions, and Open Loops`
1. `Reusable Artifacts`
1. `Provenance, Uncertainty, and Gaps`
1. Requested optional extensions
...
1. Inaccessible or partial sources were disclosed.
1. Durable knowledge was extracted with sufficient standalone context.
1. Provenance, attribution, epistemic state, and time scope remain clear.
1. Decisions, constraints, failures, relationships, and open loops were not
   flattened into generic summary language.
1. Genuine duplicates were reconciled and contradictions preserved.
1. Sensitive content was handled within appropriate boundaries.
1. The standard output contract or requested template was satisfied.
1. No unauthorized routing or mutation occurred.
1. The result can support later synapse creation without reopening the source
