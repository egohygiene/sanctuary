...
1. Inspect every accessible input and register anything that could not be
   inspected.
1. Classify candidate material before creating synapses.
1. Create only candidates that satisfy the synapse test in Section 6.
1. Use the standard synapse contract in Section 10.
1. Preserve provenance, uncertainty, and the distinction between evidence,
   inference, metaphor, and personal experience.
1. Return proposed files and a processing report. Do not write, move, merge,
...
1. **Focus:** Its essential connection can be expressed in one to three clear
   sentences.
1. **Independence:** It can be understood without reopening the original
   source, once minimum context is included.
1. **Durability:** It is likely to remain useful beyond the immediate
   conversation or task.
1. **Generativity:** It can support reflection, application, further inquiry,
   or transformation into at least one meaningful output.
1. **Integrity:** Its certainty, provenance, and boundaries can be represented
   honestly.
...
1. `split`: Create a knowledge note and one or more focused synapses, retaining
   bidirectional provenance.
1. `reclassify-knowledge`: Move or propose moving primarily factual,
   explanatory, reference, or research material into knowledge.
1. `merge`: Consolidate a genuine duplicate or variant into one canonical
   synapse while preserving unique content and source trails.
1. `flag-review`: Preserve the file and flag factual, medical, scientific,
   safety, privacy, authorship, or classification concerns.
1. `leave-unchanged`: Preserve uncertain or already-valid material pending
   human judgment.
...
1. Candidate material was classified before creation.
1. Accepted synapses satisfy the standard contract and validation checklist.
1. Provenance, uncertainty, relationships, and safety boundaries are explicit.
1. Non-synapse material was routed or reported rather than forced into the
   template.
1. Duplicate and migration risks were identified.
1. The result can be reviewed and reused without reopening the original source
   for basic context.
1. No mutation outside the user's authorization occurred.
